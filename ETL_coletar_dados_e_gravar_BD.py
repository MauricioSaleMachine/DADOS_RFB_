        ...
        for attempt in range(Config.MAX_RETRIES):
            try:
                self.connection = mysql.connector.connect(
                    ...
                    password=os.getenv('DB_PASSWORD', 'M@mu2115'),
                    database=os.getenv('DB_NAME', 'dados_rfb'),
                    charset='utf8mb4',
                    collation='utf8mb4_unicode_ci',
                    connection_timeout=30,
                    # Opções para resolver problemas de autenticação
                    auth_plugin='mysql_native_password',  # Força uso do método antigo
                    use_pure=True  # Usa o conector Python puro
                )
...
def main():
    """Função principal."""
    loader = RFBDataLoader('D:\\DADOS_RFB\\DADOS_RFB\\code')
    loader.run()
